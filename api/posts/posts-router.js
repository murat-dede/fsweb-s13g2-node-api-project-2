...
router.get("/:id", (req, res) => {
    Post.findById(req.params.id).then(findPost => {
        if (!findPost) {
            res.status(404).json({ message: "Belirliten ID'li gönderi bulunamadı." });
        } else {
            res.json(findPost);
        }
    }).catch(err=>{
...
router.post("/",async (req,res)=>{
    ...
    if(!title||!contents){
        res.status(400).json({message:"Lütfen gönderi icin bir title ve contents sağlayın"});
    }else{try {
...
router.put("/:id",async(req,res)=>{
    ...
    if(!existPost){
        res.status(404).json({message:"Verilen ID'li gönderi bulunamadı"});
    }else{
        let{title,contents}=req.body;
        if(!title||!contents){
            res.status(400).json({message:"Lütfen gönderi icin title ve contents sağlayın"});
        }else{
            try {
...
try {
    ...
    res.status(500).json({message:"Yorumlar bilgisi getirelemedi"});
}
});
